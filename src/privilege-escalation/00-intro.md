...
exploit — by invoking the legitimate elevation machinery every OS ships. The roadmap flags
this part as **more divergent** than Execution and Persistence: the three OSes guard that
boundary with very different brokers, and that divergence is the lesson.

## The one divergence to hold in your head

The cut is the same everywhere — **cross the privilege boundary through a sanctioned broker** —
but each OS models privilege, and the broker, differently:
...
So the Part III story is a **symmetric** one: each OS exposes the euid/integrity transition at the
...
| [TCC & privileged helpers](03-tcc-helpers.md) | macOS consent + root-helper / XPC abuse | — · — · TCC + SMJobBless / XPC |
