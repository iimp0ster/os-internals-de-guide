...
<div class="chapter-meta"><div class="attack-techniques"><span class="chapter-meta-label">ATT&amp;CK</span><a class="attack-badge" href="https://attack.mitre.org/techniques/T1548/"><span>Elevation control</span><code>T1548</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1068/"><span>Privilege escalation exploit</span><code>T1068</code></a></div><div class="chapter-meta-details"><span><b>Tactic</b> Privilege escalation</span><span><b>Chokepoint</b> privileged daemon authorization decision</span></div></div>
...
runs inside the daemon, so the attacker's own process may never change its credentials at all, which is exactly what makes this cut hard to see.
...
<article class="threat-use-card os-linux"><span class="threat-use-chip">LINUX</span><h3>PwnKit and CVE-2021-3560</h3><p><strong>What happens:</strong> PwnKit reaches root through <code>pkexec</code> before PolKit decides. CVE-2021-3560 tricks <code>polkitd</code> into authorizing an admin action.</p><p><strong>Detect here:</strong> Separate the front-end exploit from a broker request. They leave different evidence and require different coverage.</p><p class="threat-use-source"><a href="https://blog.qualys.com/vulnerabilities-threat-research/2022/01/25/pwnkit-local-privilege-escalation-vulnerability-discovered-in-polkits-pkexec-cve-2021-4034">Source</a></p></article>
<article class="threat-use-card os-macos"><span class="threat-use-chip">MACOS</span><h3>SecurityAgent and AMOS</h3><p><strong>What happens:</strong> A malicious login plugin captures credentials, or AMOS asks for an admin right after a fake dialog.</p><p><strong>Detect here:</strong> Monitor changes to authorization mechanisms and unusual clients asking for admin rights.</p><p class="threat-use-source"><a href="https://www.xorrior.com/persistent-credential-theft/">Source</a></p></article>
<article class="threat-use-card os-windows"><span class="threat-use-chip">WINDOWS</span><h3>PowerPool and SCShell</h3><p><strong>What happens:</strong> Privileged Windows services or RPC endpoints perform an action for the wrong caller.</p><p><strong>Detect here:</strong> The useful evidence is the privileged result, such as an altered ACL or a new service, paired with the untrusted client.</p><p class="threat-use-source"><a href="https://www.welivesecurity.com/2018/09/05/powerpool-malware-exploits-zero-day-vulnerability/">Source</a></p></article>
</div>
...
and SELinux-enforcing harden it further. **Displaced:** the goal (drive a root daemon) survives, to the next `polkitd`/D-Bus auth bug (a recurring class), to policy-config weakening
...
| Graph element | <img class="os-table-icon" src="../assets/os/linux-pixel-penguin.png" alt=""> Linux: EDR / SIEM | <img class="os-table-icon" src="../assets/os/macos-pixel-apple.png" alt=""> macOS: EDR / SIEM | <img class="os-table-icon" src="../assets/os/windows-pixel-panes.png" alt=""> Windows: EDR / SIEM |
|---|---|---|---|
...
# file.path : "/*GCONV_PATH*"). CVE-2021-3560 (the BROKER decision bypass) leaves NO attacker exec, # it is NOT expressible as one Sigma rule; author against its daemon side effects instead:
...
**Rule match:** the emulated exploit has an empty first argument, unlike the normal `pkexec --version` baseline.
