...
> **ATT&CK:** T1036.011 (Overwrite Process Arguments) · T1036.009 (Break Process Trees, related)  ·
> **Tactic:** Defense Evasion  ·  **Chokepoint:** the gap between what `execve()` captured and what
> `/proc` reports afterward  ·  **Status:** draft — capture pending (no live-lab validation yet;
> §6 rules are `experimental`)
...
process's stack at `execve()` time. That vector lives in the process's own address space —
readable, and on Linux, **writable by the process itself**. A process that controls its own
...
- **BPFDoor (S1161)** — a long-running Linux passive backdoor linked to China-based threat
  actors, first seen 2021. It "overwrites the `argv[0]` value used by the Linux `/proc`
  filesystem to determine the command line and command name to display for each process,"
  selecting names that match real system daemons: `/sbin/udevd -d`, `dbus-daemon --system`,
  `avahi-daemon: chroot helper`, `/sbin/auditd -n`, `/usr/lib/systemd/systemd-journald`.
  ([MITRE ATT&CK T1036.011](https://attack.mitre.org/techniques/T1036/011/),
  [BPFDoor](https://attack.mitre.org/software/S1161/)) BPFDoor pairs this with process-tree
  manipulation (T1036.009) and signal interception (T1564.011) — argument spoofing is one layer
  in a broader effort to not look like a process at all.
- **Self-re-exec via `/proc/self/exe`** — a distinct mechanism from BPFDoor's in-place memory
  write: a process calls `execv("/proc/self/exe", argv)` with `argv[0]` already rewritten before
  the call, most often to a bracket-syntax name (`[kworker/1:4]`) that mimics the `ps` display
  convention for kernel worker threads. This is common enough as a "how to hide a loader" idiom
  that a public reference implementation exists ([jay16/psf](https://github.com/jay16/psf)).
...
```admonish warning title="CAPTURE PENDING"
Neither rule below has been fired against a live capture yet — no caplab wave has covered this
chapter. Both are marked `experimental`, drafted from the documented `auditd` EXECVE schema and
the kernel-thread invariants in §4, not from an observed event. Per [methodology](../methodology.md)
a rule graduates to `test` only after it fires on a real event and stays silent on a benign
baseline.
```
...
detection:
  ...
  filter_systemd_reexec:
    ...
    # renames argv0 to "@systemd" or similar service-manager convention — never bracket syntax —
    # so this filter narrows on the exec target, not a blanket allow for all self-reexec
...
status: experimental
logsource: { product: linux, category: process_correlation }  # requires a stateful host agent —
                                                                # not expressible as a single
                                                                # auditd/Sigma rule
detection:
...
# Building a benign test harness for this is future lab work — not reproduced here.
~~~
```
