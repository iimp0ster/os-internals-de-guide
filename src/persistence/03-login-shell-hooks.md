...
> **ATT&CK:** T1547.001 (Registry Run Keys / Startup) · T1547.013 (XDG Autostart) · T1547.015 (Login Items) · T1546.004 (Unix Shell Config)  ·  **Tactic:** Persistence  ·  **Chokepoint:** writing the autostart entry  ·  **Status:** draft — Linux §6 detections fired on live capture (2026-06-26, caplab Wave-1); Windows & macOS §6 detections `unverified:` pending their own captures
...
- **Windows:** Emotet drops an `HKCU\...\Run` value; **FIN7 edits the Run key directly** to
  dodge process/service audit; Qbot plants a `.lnk` in the Startup folder.
  ([MITRE T1547.001](https://attack.mitre.org/techniques/T1547/001/))
- **Linux:** Linux Rabbit hooks `~/.bashrc` and `/etc/rc.local`; Raspberry Robin writes an
  `~/.config/autostart/*.desktop` entry; Kinsing drops a `/etc/profile.d/` script.
  ([MITRE T1546.004](https://attack.mitre.org/techniques/T1546/004/), [Microsoft/RaspberryRobin](https://www.microsoft.com/security/blog/2022/07/27/raspberry-robin-worm-part-of-larger-ecosystem-pushing-initial-access-brokers-as-a-service/))
- **macOS:** Green Lambert modifies shell rc; Adload uses Login Items (and hybridizes with
  shell rc to survive removal of one). ([MITRE T1547.015](https://attack.mitre.org/techniques/T1547/015/), [Objective-See](https://theevilbit.github.io/beyond/))
...
**Linux** rules captured & validated (2026-06-26, caplab Wave-1). **Windows Run key rule
captured & validated** (2026-06-28, caplab Wave-2): EID 13 on `HKU\.DEFAULT\...\Run` fired on
`reg.exe` write, baseline clean. **macOS** rule remains `unverified:` — deferred to macOS capture.
```
...
status: test   # reconciled vs capture (2026-06-28, caplab Wave-2, login03-runkey): EID 13 fired on
...
level: medium
# RECONCILED vs capture (2026-06-26, caplab Wave-1): the rule fired on `comm=bash` opening `/root/.bashrc`
