...
> **ATT&CK:** T1053.002 (At) · T1053.003 (Cron) · T1053.005 (Scheduled Task) · T1053.006 (Systemd Timers)  ·  **Tactic:** Persistence/Execution  ·  **Chokepoint:** writing the scheduler's config  ·  **Status:** draft — Linux §6 detections fired on live capture (2026-06-26, caplab Wave-1); Windows + macOS `unverified:` pending their own captures
...
- **Windows tasks:** Emotet (`schtasks /create /ru SYSTEM /sc MINUTE /mo 1` — beacon every
  minute); APT29 created tasks via **COM** to avoid `schtasks.exe`; Wizard Spider/TrickBot
  **inject directly into the TaskCache registry**; Tarrask abused tasks and *hid* them.
  ([MITRE T1053.005](https://attack.mitre.org/techniques/T1053/005/), [Microsoft/Tarrask](https://www.microsoft.com/en-us/security/blog/2022/04/12/tarrask-malware-uses-scheduled-tasks-for-defense-evasion/))
- **Linux cron/timers:** LemonDuck and Kinsing (cloud cryptominers) write user crontabs and
  drop systemd `.timer` units to re-pull payloads. ([Aqua](https://www.aquasec.com/blog/), [MITRE S0949](https://attack.mitre.org/software/S0949/))
- **macOS launchd:** Silver Sparrow ran an **hourly** LaunchAgent; most macOS persistence is
  actually `RunAtLoad` (boot/login) rather than pure interval — which itself reinforces that
  scheduling and services are one subsystem here. ([Red Canary](https://redcanary.com/blog/threat-intelligence/clipping-silver-sparrows-wings/))
...
```admonish warning title="CAPTURE PENDING — macOS only (Windows partial)"
**Linux** rules captured & validated (2026-06-26, caplab Wave-1). **Windows TaskCache complement
rule captured & validated** (2026-06-28, caplab Wave-2, sched02-taskcache): Sysmon EID 13 on
`\Schedule\TaskCache\Tree\*` + EID 11 on `\System32\Tasks\*` fired on direct-registry injection
with no EID 106 (evasion confirmed). EID 106 primary rule and schtasks/SYSTEM variant remain
`unverified:` — deferred. **macOS** rule deferred.
```
...
status: test   # reconciled vs capture (2026-06-28, caplab Wave-2, sched02-schtasks): all three anchors
               ...
               #   NOTE: UserContext may be emitted as SID (S-1-5-18) not display name (SYSTEM) —
               #   match on either: UserContext|contains 'SYSTEM' OR UserContext = 'S-1-5-18'.
...
status: test   # reconciled vs capture (2026-06-28, caplab Wave-2, sched02-taskcache): EID 13 fired
