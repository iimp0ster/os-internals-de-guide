...
<div class="chapter-meta"><div class="attack-techniques"><span class="chapter-meta-label">ATT&amp;CK</span><a class="attack-badge" href="https://attack.mitre.org/techniques/T1505/003/"><span>Web shell</span><code>T1505.003</code></a></div><div class="chapter-meta-details"><span><b>Tactic</b> Persistence</span><span><b>Chokepoint</b> web server becomes the parent of a shell or recon child</span></div></div>
...
<article class="threat-use-card os-cross"><span class="threat-use-chip">WEB SERVER</span><h3>China Chopper</h3><p><strong>What happens:</strong> A small ASPX, PHP, or JSP stub accepts commands through an HTTP request.</p><p><strong>Detect here:</strong> The shell code changes by family. A web server spawning a command interpreter does not.</p><p class="threat-use-source"><a href="https://media.defense.gov/2020/Jun/09/2002313081/-1/-1/0/CSI-DETECT-AND-PREVENT-WEB-SHELL-MALWARE-20200422.PDF">Source</a></p></article>
<article class="threat-use-card os-cross"><span class="threat-use-chip">WEB SERVER</span><h3>Generic app web shells</h3><p><strong>What happens:</strong> A file upload or application flaw leads to a PHP, JSP, or ASPX shell running recon commands.</p><p><strong>Detect here:</strong> Parent-child lineage generalizes across delivery bugs and shell families. Start with web server to shell, then web server to recon tool.</p><p class="threat-use-source"><a href="https://www.acunetix.com/blog/articles/web-shells-101-using-php-introduction-web-shells-part-2/">Source</a></p></article>
</div>
...
telemetry: `auditd`'s EXECVE record has the child's argv but only a bare `ppid` for the parent, resolving that to `/usr/sbin/nginx` for the `ParentImage|endswith` match the rule below needs
...
The gap is entirely in resolving *which binary* the numeric PPID points to at the SIEM tier, the data to do it exists (every process's own EXECVE record has its exe path), it just isn't
joined for you the way an EDR-tier sensor joins it inline.

## 6. Detect the cut

```admonish info title="Upstream rule provenance"
...
lineage-anomaly rule. SigmaHQ's `status: test` is upstream metadata; validate the rule against
your own process-creation telemetry before deployment.
```

```yaml
title: Linux Webshell Indicators
id: 818f7b24-0fba-4c49-a073-8b755573b9c7
status: test                                            # SigmaHQ upstream status
