...
> **ATT&CK:** T1543.001 (Launch Agent) · T1543.002 (Systemd Service) · T1543.003 (Windows Service) · T1543.004 (Launch Daemon)  ·  **Tactic:** Persistence  ·  **Chokepoint:** writing the service-manager's config  ·  **Status:** draft — §6 Linux detections fired on live capture (2026-06-26); Windows/macOS `unverified:` pending their own captures
...
- **Windows services:** Emotet and TrickBot register svchost-hosted services (DLL via
  `ServiceDll`); FIN7/Carbanak **hijack existing** services by editing the registry directly
  (evading the install event). ([Elastic](https://www.elastic.co/security-labs/emotet-back-for-business), [Mandiant](https://www.mandiant.com))
- **Linux systemd:** LemonDuck and Kinsing (cloud cryptominers) drop `.service` units named to
  mimic system services, `Restart=always`, run as root. ([MITRE S0949](https://attack.mitre.org/software/S0949/), [Aqua](https://www.aquasec.com/blog/))
- **macOS launchd:** Atomic Stealer (AMOS), Odyssey, and Silver Sparrow persist via
  LaunchDaemons/LaunchAgents with `RunAtLoad`. ([Red Canary](https://redcanary.com/blog/threat-intelligence/atomic-odyssey-poseidon-stealers/), [Censys](https://censys.com/blog/odyssey-stealer-inside-a-macos-crypto-stealing-operation/))
- **Cross-platform RaaS** (e.g. LockBit) ships service persistence for *both* Windows and Linux.
...
Linux has none; macOS recently grew one but it's shaky.** And on every OS the *durable* signal
— the config write — is strong at the EDR tier and weak-to-absent at the SIEM tier off-Windows.

## 6. Detect the cut

```admonish warning title="CAPTURE PENDING — macOS only"
**Linux** rules captured & validated (2026-06-26, caplab Wave-1). **Windows complement rule
(direct registry edit) captured & validated** (2026-06-28, caplab Wave-2, svc01-reg-service):
Sysmon EID 13 on `\Services\*\ImagePath` fired via direct `New-ItemProperty` (no SCM), baseline
clean. Primary 7045 rule remains `unverified:` (SCM path not exercised in this cell).
**macOS** rule deferred.
```
...
status: test   # reconciled vs capture (2026-06-28, caplab Wave-2, svc01-reg-service): EID 13 fired
