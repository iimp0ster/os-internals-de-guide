...
<div class="chapter-meta"><div class="attack-techniques"><span class="chapter-meta-label">ATT&amp;CK</span><a class="attack-badge" href="https://attack.mitre.org/techniques/T1620/"><span>Reflective loading</span><code>T1620</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1055/"><span>Process injection</span><code>T1055</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1055/012/"><span>Process hollowing</span><code>T1055.012</code></a></div><div class="chapter-meta-details"><span><b>Tactic</b> Execution / Defense evasion</span><span><b>Chokepoint</b> executable memory with no file image</span></div></div>
...
<article class="threat-use-card os-windows"><span class="threat-use-chip">WINDOWS</span><h3>Beacon and Qakbot</h3><p><strong>What happens:</strong> Reflective loading or PE injection moves code into <code>explorer.exe</code> or another trusted process.</p><p><strong>Detect here:</strong> A private memory region becomes executable, then a remote thread or suspicious start address runs from it.</p><p class="threat-use-source"><a href="https://www.mandiant.com/m-trends">Source</a></p></article>
<article class="threat-use-card os-windows"><span class="threat-use-chip">WINDOWS</span><h3>TrickBot and Emotet</h3><p><strong>What happens:</strong> Process hollowing replaces the image of a suspended <code>svchost.exe</code>.</p><p><strong>Detect here:</strong> The process name says service host. Its image and early memory activity say otherwise.</p><p class="threat-use-source"><a href="https://www.cisa.gov">Source</a></p></article>
<article class="threat-use-card os-linux"><span class="threat-use-chip">LINUX</span><h3>Mirai memfd loaders</h3><p><strong>What happens:</strong> An ELF runs from an anonymous memory file instead of disk.</p><p><strong>Detect here:</strong> Capture <code>memfd_create</code> followed by <code>execveat</code>. File scanning cannot see this chain.</p><p class="threat-use-source"><a href="https://www.elastic.co/security-labs/a-deep-dive-into-linux-rootkits">Source</a></p></article>
<article class="threat-use-card os-macos"><span class="threat-use-chip">MACOS</span><h3>macOS injection attempts</h3><p><strong>What happens:</strong> Higher-capability tooling targets unsigned binaries. Commodity stealers usually avoid this route.</p><p><strong>Detect here:</strong> Treat any unsigned target with injected code as high priority. Platform controls make it less common.</p><p class="threat-use-source"><a href="https://unit42.paloaltonetworks.com">Source</a></p></article>
</div>
...
| anonymous in-memory file | n/a (PE in memory, not a file) | `memfd_create`, eBPF ✅ / auditd ❌ default | n/a (Mach-O in memory, HR-restricted) |
...
status: test                                                # validated against a captured event
...
**Rule match:** `execveat` carries `AT_EMPTY_PATH` (`0x1000`), which identifies execution by file descriptor rather than a filesystem path.
