...
> **ATT&CK:** T1059.004 (Unix Shell) · T1059.006 (Python) · T1059.002 (AppleScript)  ·  **Tactic:** Execution  ·  **Chokepoint:** the interpreter `exec` edge  ·  **Status:** draft — Linux §6 detections fired on live capture (2026-06-26, caplab Wave-1); Windows + macOS `unverified:` pending their own captures
...
- **Download-and-execute cradles** — `curl … | bash` (Linux/macOS), `IEX (New-Object
  Net.WebClient).DownloadString(...)` (Windows). Ubiquitous in commodity intrusions
  (coin-miner drops, post-RCE staging).
- **macOS infostealers (Atomic Stealer / AMOS family)** — abuse `osascript` with
  AppleScript `display dialog` to throw fake password prompts for credential theft
  ([T1059.002](https://attack.mitre.org/techniques/T1059/002/)). *macOS-only realization.*
- **Web-shell / RCE** — a compromised web application spawns `python -c` / `bash -c` as
  a child of the web-server process. High signal: the *parent* is anomalous.
...
The **Linux** rules below are captured & validated: both fired on a real event (shown
inline, `status: test`) and stayed silent on a benign baseline (caplab Wave-1, 2026-06-26).
The **Windows** and **macOS** rules remain **`unverified:`** — logic drafted from documented
event schemas, not yet fired against a real captured event. Per [methodology](../methodology.md),
a rule is done only when it fires on a real event (shown inline) and **not** on a benign
baseline; those schema blocks are illustrations, replaced with redacted live events after
their own lab captures.
```
