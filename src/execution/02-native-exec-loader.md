...
<div class="chapter-meta"><div class="attack-techniques"><span class="chapter-meta-label">ATT&amp;CK</span><a class="attack-badge" href="https://attack.mitre.org/techniques/T1129/"><span>Shared modules</span><code>T1129</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1574/001/"><span>DLL search order</span><code>T1574.001</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1574/002/"><span>DLL side-loading</span><code>T1574.002</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1574/004/"><span>Dylib hijacking</span><code>T1574.004</code></a><a class="attack-badge" href="https://attack.mitre.org/techniques/T1574/006/"><span>Dynamic linker</span><code>T1574.006</code></a></div><div class="chapter-meta-details"><span><b>Tactic</b> Execution</span><span><b>Chokepoint</b> loader resolves and maps a library</span></div></div>
...
<article class="threat-use-card os-windows"><span class="threat-use-chip">WINDOWS</span><h3>PlugX and ShadowPad</h3><p><strong>What happens:</strong> A signed program loads an attacker DLL placed beside it.</p><p><strong>Detect here:</strong> Trust the module path less than the signer. A signed executable loading an unsigned local DLL is the useful mismatch.</p><p class="threat-use-source"><a href="https://www.elastic.co/security-labs/dll-sideloading-supply-chain-attacks">Source</a></p></article>
<article class="threat-use-card os-linux"><span class="threat-use-chip">LINUX</span><h3>Symbiote and HiddenWasp</h3><p><strong>What happens:</strong> <code>LD_PRELOAD</code> or <code>ld.so.preload</code> injects a shared object into other processes.</p><p><strong>Detect here:</strong> Preload configuration changes and unexpected loader environment variables expose the setup before the hidden process runs.</p><p class="threat-use-source"><a href="https://www.intezer.com/blog/research/new-linux-threat-symbiote/">Source</a></p></article>
<article class="threat-use-card os-macos"><span class="threat-use-chip">MACOS</span><h3>AMOS dylib injection</h3><p><strong>What happens:</strong> Atomic Stealer targets unsigned or cracked apps and loads a dylib into a browser.</p><p><strong>Detect here:</strong> A browser loading a library from a user-writable or mounted location is unusual, even when the browser itself looks normal.</p><p class="threat-use-source"><a href="https://redcanary.com/threat-detection-report/">Source</a></p></article>
</div>
...
after the system/Windows directories, so legitimate system DLLs resolve before CWD is reached, suppressing the classic CWD-relative search-order hijack (T1574.001) and **displacing attackers
...
status: test                                           # validated against the captured event below
...
**Rule match:** the artifact write matches the preload-path arm; the execution matches the `env` wrapper and `LD_PRELOAD=` argument arm.
...
The benign baseline did not trigger the rule.
```
