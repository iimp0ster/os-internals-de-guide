...
| Observe | Windows | Linux — EDR tier | Linux — SIEM tier | macOS — EDR tier | macOS — SIEM tier |
|---|---|---|---|---|---|
...
| Full argv / command line | EID 1 `CommandLine` (single string) | full `argv[]` array | `EXECVE` record `a0..aN` (**can truncate**) | ESF `es_exec_arg` (full) | — |
| Parent lineage | EID 1 `ParentImage`/`ParentProcessGuid` | parent pid + exe in event | `ppid` (needs correlation) | ESF parent `audit_token` + path | — |
| Fork (no exec) | (N/A on Windows model) | eBPF `sched_process_fork` | auditd `clone`/`fork` | ESF `ES_EVENT_TYPE_NOTIFY_FORK` | — |
...
| Observe | Windows | Linux — EDR | Linux — SIEM | macOS — EDR | macOS — SIEM |
|---|---|---|---|---|---|
